...
import {
	setCurrentActorError,
	setCurrentActorLoading,
	selectCurrentActorLoading,
	selectCurrentActorError,
	selectApiKey,
	selectCurrentActorResult,
	setCurrentActorResult,
	selectCurrentActorImages,
	setCurrentActorImages,
	selectCurrentActorCast,
	setCurrentActorCast,
...
const ActorPage = () => {
	...
	useEffect(() => {
		...
		fetch(
			...
			.then((res) => res.json())
			.then((res) => dispatch(setCurrentActorImages(res.profiles)))
			.catch((error) => console.log(error))
		fetch(
			`https://api.themoviedb.org/3/person/${personId}/movie_credits?api_key=${apiKey}`
		)
			.then((res) => res.json())
			.then((res) => dispatch(setCurrentActorCast(res.cast)))
			.catch((error) => console.log(error))
	...
	if (error) {
		return <h1 className='text-danger'>Sorry, something went wrong...</h1>
	}
				...
				<div className='actor-text'>
					...
					<div className='actor-text__header flex-column align-items-start'>
						...
						<div className='d-flex flex-column' style={{ textAlign: 'left' }}>
							...
							<span>
								Rate: {actor.popularity.toFixed(1)} <AiFillStar />
							</span>
						</div>
					</div>
